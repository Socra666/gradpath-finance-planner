(function(){
  ...
  </div><aside class="sidecol"><div class="side"><div class="school">申请关键节点</div><div class="deadline">${esc(p.deadline)}</div><div class="sub">${esc(p.term)}</div>${officialButtons()}<a class="official secondary" href="documents.html?project=${p.id}">打开该项目资料夹 →</a><button class="save" id="saveBtn">${saved.has(p.id)?'★ 已收藏':'☆ 收藏项目'}</button><h3>我的材料进度</h3>${progressItems.map((x,i)=>`<label class="check"><input type="checkbox" data-check="${i}" ${localStorage.getItem('detail-precise-check-'+p.id+'-'+i)==='1'?'checked':''}><span>${esc(x)}</span></label>`).join('')}</div></aside></div>`;
